...
(async () => {
  ...
  const jobId = await sha256(encode({companies: config.companies, years: config.years, periods: config.periods || [4], items: config.items, batchSize: config.batchSize}));
  ...
  const run = async () => navigator.locks.request("kap-export-host-v6", {ifAvailable: true}, async lock => {
    ...
    for (let index = 0; index < batches.length; index++) {
      ...
      const payload = {companyType: "IGS", mkkMemberIdList: companies.map(c => c.source_company_id),
        mkkMemberTitleList: companies.map(c => c.company_name), yearList: config.years.map(String), periodList: (config.periods || [4]).map(String),
