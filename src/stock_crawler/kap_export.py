...
from .models import CompanyIdentity, ConsolidationScope, FilingCandidate, FilingDownload, FundamentalRecord, ParsedReport, ParseStatus
...
def read_export(data: bytes) -> list[dict]:
            ...
            for number, cells in enumerate(sheet.iter_rows(), 1):
                ...
                if headers is None:
                    if values[:len(META_HEADERS)] == list(META_HEADERS):
                        ...
                        if len(set(headers)) != len(headers) or any(h not in (*META_HEADERS, *FIELDS) for h in headers):
