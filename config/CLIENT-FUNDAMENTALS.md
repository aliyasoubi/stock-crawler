...
| Revenue | `Revenue` | Reported; presentation scale applied once. |
...
`CLIENT-FUNDAMENTALS.md` guide. If you changed these files since the reviewed
...
- Default scope is consolidated. Use `--scope unconsolidated` only when that is
  the requested basis; there is no silent fallback from one scope to another.
...
- Nonannual and unreviewed sector statements, unknown company identities,
  formulas, unknown workbook headers and invalid parsed records are rejected.
