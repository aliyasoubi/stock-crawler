...
from .test_kap_export import BOOK, REGISTRY, _mock_export_client, modified_book
...
def test_missing_requested_year_is_visible_and_not_marked_fresh(settings, repo, clock):
    def change(sheet):
    ...
    assert len(requests) == 2 and all(r['status'] != 'fresh' for r in second.data['companies'])
    fetcher.close()
...
def test_parser_change_requires_new_coverage(settings, repo, clock):
    requests = []
    source, state, fetcher = _mock_export_client(settings, clock, BOOK.read_bytes(), requests)
    pipeline = Pipeline(settings, repo, RawStore(settings.data_dir), state, source, clock=clock)
    ...
    pipeline.parser_version = '1.2.0'
    assert all(r['status'] == 'published' for r in pipeline.sync(SyncOptions(tickers=['ASELS', 'THYAO'])).data['companies'])
    assert len(requests) == 2
    fetcher.close()
