...
from .db import IdentityConflict, RepositoryLike
...
from .models import ANNUAL_PERIOD, CompanyIdentity, FilingCandidate, FundamentalRecord, ParsedReport, ParseStatus, ReportRecord
from .parser import PARSER_VERSION, parse_snapshot
from .storage import Clock, RawStore, RunSummary, SnapshotMissing, SnapshotRef, StateStore, StorageError, dump_json, utcnow
...
class Pipeline:
    ...
    def _is_fresh(self, company: Any, now: datetime) -> bool:
        """Recent AND already covering every requested fiscal year.
        ...
        if not getattr(self.source, "publishes_all_years", False):
            return True  # notification-style sources select the latest filing, not a year set
    ...
    def _coverage_key(self, company: Any) -> str:
        return f"{self.source.market_source}/{company.source_company_id}/{self.parser_version}"
    ...
    def _sync_company(self, identity: CompanyIdentity, company: Any) -> list[dict[str, Any]]:
        ...
        for requested_year, selection in selections:
            ...
            if getattr(self.source, "publishes_all_years", False) and entry.get("status") in ("published", "already_parsed"):
                self.state.add_coverage(self._coverage_key(company), [selected.fiscal_year])
            entries.append(entry)
        return entries
