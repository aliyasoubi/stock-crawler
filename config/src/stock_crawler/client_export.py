...
def build_export(paths, *, settings: Settings, company_map=None, currency='TRY',
                 scope='consolidated', net_income_basis='total-profit', tickers=None):
    if not re.fullmatch(r'[A-Z]{3}', currency):
        raise SettingsError('currency must be a three-letter uppercase code, e.g. TRY')
    if scope not in ('consolidated', 'unconsolidated'):
        raise SettingsError('scope must be consolidated or unconsolidated')
    ...
    for path in sorted(set(Path(p) for p in paths)):
        ...
        for row in rows:
            ...
            try:
                ...
                if parsed.consolidation_scope.value != scope:
                    skipped_scope += 1
                    continue
                ...
                rank = (candidate.published_at, int(candidate.notification_id))
    ...
    return {
        ...
        'missing_required_field_counts': dict(counts), 'errors': errors,
...
def cmd_export_company_fundamentals(args):
    ...
    print(f"Client review: {report['selected_records']} records; "
          ...
          f"{len(report['errors'])} source errors; 0 HTTP attempts")
