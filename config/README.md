...
The supplied archive contains `env.example`, but `scripts/setup-linux.sh` expects
`.env.example`. Create the missing template name before invoking setup:
...
`local.env` is not loaded automatically. This guide uses `.env` throughout.
...
- Do not add `--refresh` routinely. `fresh` means the company is skipped because
  of its recent discovery status; check stored rows to confirm year coverage.
- Freshness is tracked per company, not per requested year set. After deliberately
  changing `KAP_YEARS`, wait for the interval to expire or use `--refresh` for the
  intended selection. Changing the years does not automatically bypass freshness.
...
| `no_filing`, `unresolved`, `error`, `failed`, `unsupported` | Inspect the corresponding summary error. |
...
| `config/kap_companies.json` | KAP identities and title aliases |
...
See [REVIEW.md](REVIEW.md) for earlier data-quality and implementation decisions.
