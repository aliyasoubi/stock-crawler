...
from stock_crawler.crawl.kap import SourceError, UnknownTicker
from stock_crawler.crawl.kap_export import (CompanyRegistry, ExportEntry, KapExportClient, archive_export, export_payload,
    load_manifest, parse_export_row, read_export, read_export_blob)
from stock_crawler.core.storage import RawStore, StateStore, StorageError
from stock_crawler.crawl.fetch import PacedClient
from stock_crawler.crawl.pipeline import Pipeline, SyncOptions
from stock_crawler.crawl.parser import PARSER_VERSION
...
def test_normalized_key_never_merges_distinct_issuers():
    from stock_crawler.crawl.kap_export import loose_key
    registry = CompanyRegistry(REGISTRY)
    keys = {}
...
def test_holding_format_rows_publish_with_both_revenue_lines_kept_apart():
    ...
    from stock_crawler.crawl.kap_export import HOLDING_REVENUE_SPLIT
    rows = read_export(BOOK.read_bytes())
...
def test_general_format_with_only_finance_sector_revenue_is_valid_and_flagged():
    ...
    from stock_crawler.crawl.kap_export import REVENUE_FINANCE_ONLY
    rows = read_export(BOOK.read_bytes())
